...
function opcoesSelect() {
  ...
  for (var i = 0; i < curso.length; i++) {
    if (!vetCursosLista.includes(curso[i])) {
      vetCursosLista.push(curso[i]);
    }
  }
...
function funSelect() {
  ...
  for (var i = 0; i < aluno.length; i++) {
    let nomeAluno = aluno[i];
    let cursoAtual = curso[i];
    let resultProva1 = prova1[i];
    let resultProva2 = prova2[i];
    let resultParticipacao = participacao[i];
    let numFaltas = falta[i];
